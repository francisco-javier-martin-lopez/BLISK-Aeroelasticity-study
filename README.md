...
* **Non-Linear Aerodynamic Simulations:** A deeper dive into the influence of aerodynamic non-linearities on compressor damping. The repository includes one of the main progress reports from this phase, which highlights the initial non-linear results, mesh refinement iterations (including tip-gap integration), and domain modifications to benchmark the solver against experimental (EXP) and the linear in-house solver reference data.
